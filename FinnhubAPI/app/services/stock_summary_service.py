...
from database.models import StockSummary
...
class StockSummaryService():
    def __init__(self, CustomModel: type[BaseModel]):
        ...
        self.sync_stock_summary()
        self.data_filter_properties = {}

    def sync_stock_summary(self):
        with CassandraClient(self.cassandra_config) as cassandra_client:
            cassandra_client.sync_table(StockSummary)

    def get_stock_summary(self, batch_size: int=100, reset_start_date: int=None) -> list[type[BaseModel]] | None:
        with CassandraClient(self.cassandra_config) as cassandra_client:
            try:
                data = cassandra_client.get_all_items_by_partition_date(
                    batch_size=batch_size,
                    CustomModel=StockSummary,
