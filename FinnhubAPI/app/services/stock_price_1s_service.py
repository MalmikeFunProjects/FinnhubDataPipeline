...
class StockPrice1service():
    def __init__(self, pydantic_model: type[BaseModel]):
        self.cassandra_config = get_cassandra_config()
        self.cassandra_mapper = CassandraMapper(pydantic_model)
        self._sync_stock_price_1s_table()

    def _sync_stock_price_1s_table(self):
        """Ensure the stock_price_1s table is synced with Cassandra"""
        try:
            with CassandraClient(self.cassandra_config) as cassandra_client:
                cassandra_client.sync_table(StockPrice1sModel)
                logger.info("Stock price 1s table synced successfully")
        except Exception as e:
            logger.error(f"Failed to sync stock price 1s table: {str(e)}")
            raise

    def get_stock_price_1s(
        self,
        ...
        """
        Get stock price 1s data with various filtering options
        ...
        Returns:
            List of stock price 1s records as Pydantic models
        """
        try:
            with CassandraClient(self.cassandra_config) as cassandra_client:
                ...
                if not result or not result.get("records"):
                    logger.info(f"No stock price 1s data found for params: {filter_params}")
                    return []
            ...
            logger.exception(f"Error getting stock price 1s: {str(e)}")
            raise
