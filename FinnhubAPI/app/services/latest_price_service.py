...
from database.models import LatestPrice
...
class LatestPriceService():
    def __init__(self, CustomModel: type[BaseModel]):
        ...
        self.sync_latest_price()
        self.data_filter_properties = {"extra_filters": {}}

    def sync_latest_price(self):
        with CassandraClient(self.cassandra_config) as cassandra_client:
            cassandra_client.sync_table(LatestPrice)

    def get_latest_price(self, batch_size: int=100, symbol: str=None, reset_start_date: int=None) -> list[type[BaseModel]] | None:
        with CassandraClient(self.cassandra_config) as cassandra_client:
            try:
                if symbol:
