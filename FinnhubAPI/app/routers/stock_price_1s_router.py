...
class StockPrice1sRouter():
    """Router handling stock price 1s endpoints and WebSocket connections"""
    def __init__(self):
        self.service = StockPrice1service(StockPrice1s)
    ...
    def setup_routes(self):
        ...
        self.router.add_api_route(
            "/",
            self.get_stock_price_1s,
            methods=["GET"],
            response_model=List[StockPrice1s]
        )
    ...
    async def websocket_endpoint(
        ...
        """Handle WebSocket connections for streaming stock price 1s data"""
        # Parse connection parameters
        connection_id = f"stock_price_1s_{datetime.now().timestamp()*1000}"
        ...
        async with manager.connection_context(websocket, connection_id, metadata) as client_id:
            logger.info(f"WebSocket connection started: {client_id} (stock_price_1s)")
    ...
    async def _stream_stock_price_1s(
        self,
        client_id: str,
        ...
        """Stream stock price 1s data to the client"""
        ...
        try:
            ...
            # Send initial batch
            initial_data = self._fetch_stock_price_1s(symbol, start_days_ago)
            ...
            last_timestamp = await self._send_price_record_ws(
                ...
                stock_price_1s=initial_data
            )

            # Start continuous updates
            while True:
                # Fetch new prices since last update
                new_prices = self._fetch_stock_price_1s(
                    symbol=symbol,
    ...
    async def _send_price_record_ws(
        ...
        stock_price_1s: List[StockPrice1s]=[],
    ...
    def _fetch_stock_price_1s(
        ...
        """Fetch stock price 1s from the service layer"""
        try:
            return self.service.get_stock_price_1s(
                symbol=symbol,
                start_days_ago=start_days_ago,
            ...
            logger.exception(f"Error fetching stock price 1s: {str(e)}")
            return []
    ...
    async def get_stock_price_1s(
        ...
        try:
            prices = self._fetch_stock_price_1s(
                symbol=symbol,
                start_days_ago=start_days_ago,
                batch_size=limit
            )
            ...
            if not prices:
                raise HTTPException(
                    status_code=404,
                    detail=f"No stock price 1s found for the specified parameters"
                )
            ...
            logger.exception(f"Error in get_stock_price_1s: {str(e)}")
            raise HTTPException(
                status_code=500,
                detail="An error occurred while retrieving stock price 1s"
            )
